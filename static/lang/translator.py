...
class BackendTranslator:
    ...
    def t(self, key: str, language: Optional[str] = None, silent: bool = False, **kwargs) -> str:
        ...
        # format_map mit Default-Dict: fehlende Vars werden zu '' statt
        # KeyError + raw-template-Rueckgabe. Sonst sieht der User
        # "{icon} {stage_desc}" wenn der Caller eine Var vergessen hat.
        class _SafeDict(dict):
            def __missing__(self, key):
                return ''
        try:
            return translation.format_map(_SafeDict(kwargs))
        except Exception as e:
            logger.error(f"❌ Translation format failed for {key}: {e}")
            return translation
